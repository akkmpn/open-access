// ===== GLOBAL FILTER FUNCTION (FIX FOR UNDEFINED ERROR) =====
function filterTasks(category) {
  if (window.setTaskFilter) {
    return window.setTaskFilter(category);
  }
  console.warn('setTaskFilter not available yet');
}
    ...
    <div class="task-card priority-${task.priority} ${task.completed ? 'completed' : ''} ${isOverdue ? 'overdue' : ''}" 
      ...
      <div class="task-header">
        ...
        <div class="task-content">
          <h4 class="task-title ${task.completed ? 'completed-title' : ''}">
...
function filterAndSearchTasks(tasks) {
  ...
  if (currentFilter === 'active') {
    ...
    filtered = filtered.filter(task => 
      task.date && new Date(task.date) < today && !task.completed
    );
  }
